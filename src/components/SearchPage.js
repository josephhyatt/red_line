...
function populateModels(make, models) {
	if (make in models) {
		return models[make].map((item) => <MenuItem value={item}>{item}</MenuItem>);
	}
}
...
function Search() {
	...
	const searchForCars = () => {
		const fetchItems = () => {
			...
			if (makeVal != "") totalFilter = totalFilter.where("make", "==", makeVal);
			if (modelVal != "")
			...
			totalFilter.get().then(function (querySnapshot) {
				setItems(
					querySnapshot.docs.map((doc) => doc.data()).sort(sortByFilter)
				);
			});
		};
		fetchItems();
	...
	const sortByFilter = (a, b) => {
		if (sortBy != "") {
			...
			else if (sortBy == "oldest") return parseInt(a.year) - parseInt(b.year);
			...
			else if (sortBy == "cheap") return parseInt(a.price) - parseInt(b.price);
		}
	};
		...
		<>
			<div className="searchFilterBox">
				<FormControl variant="outlined" className={classes.formControl}>
					...
					<Select value={makeVal} onChange={handleSelectMake} label="Make">
						<MenuItem value="">
							<em>None</em>
						</MenuItem>
				...
				<FormControl variant="outlined" className={classes.formControl}>
					...
					<Select value={modelVal} onChange={handleSelectModel} label="Model">
						<MenuItem value="">
							<em>None</em>
						</MenuItem>
				...
				<FormControl variant="outlined" className={classes.formControl}>
					...
					<Select value={fuelVal} onChange={handleSelectFuel} label="Fuel">
						<MenuItem value="">
							<em>None</em>
						</MenuItem>
				...
				<FormControl variant="outlined" className={classes.formControl}>
					...
					<Select value={sortBy} onChange={handleSelectSortBy} label="sortBy">
						<MenuItem value="">
							<em>None</em>
						</MenuItem>
			...
			<GridList classname="grid" cellHeight={425} cols={"3"}>
				{items.map((item) => (
					<Card className="text-center" id="fix-grid">
						<>
